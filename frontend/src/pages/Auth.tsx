...
import { Zap, Mail, Lock, User, KeyRound, Building2, Loader2 } from "lucide-react";
import { toast } from "sonner";
import { validateSignupToken, claimSignupToken } from "@/lib/api";
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        const validation = await validateSignupToken(inviteCode.trim().toUpperCase());
        if (!validation.valid) {
          toast.error(validation.error || "Invalid invite code");
          setLoading(false);
          return;
        }
        ...
        if (error) throw error;
        // Claim the token — best effort, don't block user on failure
        try {
          await claimSignupToken(inviteCode.trim().toUpperCase());
        } catch {
          // Token claim happens server-side on first authenticated request too
        }
